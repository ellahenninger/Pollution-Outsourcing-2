...
data_s <- data_s %>%
  group_by(ccode) %>%
  mutate(
    demeaned_log_ghg = log_ghg - mean(log_ghg, na.rm = T),
    ...
    mean_log_ghg = mean(log_ghg, na.rm = T),
...
m2 <- lm(log_cc_outs ~ log_pop + log_gdp + log_gdp_pc + as.factor(year), data = data_ova)
m3 <- lm(log_cc_outs ~ vdem_polyarchy + log_pop + log_gdp + log_gdp_pc + as.factor(year), data = data_ova)
...
m5_wb <- lmer(log_ghg ~ demeaned_log_cc_outs + demeaned_vdem_poly + 
               mean_log_cc_outs + mean_vdem_poly +
               as.factor(year) + (1 | ccode),
              data = data_ova)

m6_wb <- lmer(log_ghg ~ demeaned_log_cc_outs + demeaned_vdem_poly +
...
m7_wb <- lmer(log_ghg ~ demeaned_log_pop + demeaned_log_gdp_pc + demeaned_log_gdp_pc_sq + 
               mean_log_pop + mean_log_gdp_pc + mean_log_gdp_pc_sq + 
               as.factor(year) + (1 | ccode),
              data = data_ova)

m8_wb <- lmer(log_ghg ~ demeaned_log_cc_outs + demeaned_vdem_poly + 
                demeaned_log_cc_outs * mean_vdem_poly +
                demeaned_log_pop + 
                demeaned_log_gdp_pc + demeaned_log_gdp_pc_sq +
                mean_log_cc_outs + mean_vdem_poly +
                mean_log_cc_outs * mean_vdem_poly +
                mean_log_pop +  
                mean_log_gdp_pc + mean_log_gdp_pc_sq +
                as.factor(year) + (1 | ccode),
              data = data_ova)

#-------------------------------------------------------------------------------
# Model group 2 - Original
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
pdf("Plots/Coefficients_table2_within_between_new.pdf", width = 7.5)
...
points(x = coefs_within2$Estimate,
       ...
       pch = 16,
       col = c(viridis(3)[1], viridis(3)[1],viridis(3)[1],viridis(3)[1], viridis(3)[1],viridis(3, alpha = 0.6)[1]),
       cex = 1)

# add CIs
segments(y0 = rev(c(0.25, 0.65, 1.05, 1.45, 1.85, 2.25)), 
         y1 = rev(c(0.25, 0.65, 1.05, 1.45, 1.85, 2.25)), 
         col = c(viridis(3)[1], viridis(3)[1], viridis(3)[1],viridis(3)[1], viridis(3)[1],viridis(3, alpha = 0.3)[1]),
...
points(x = coefs_between2$Estimate,
       ...
       pch = 16,
       col = c(viridis(3, alpha = 0.6)[2], viridis(3)[2],viridis(3, alpha = 0.6)[2],viridis(3)[2],
               viridis(3)[2], viridis(3, alpha = 0.6)[2]),
       cex = 1)

# add CIs
segments(y0 = rev(c(0.15, 0.55, 0.95, 1.35, 1.75, 2.15)), 
         y1 = rev(c(0.15, 0.55, 0.95, 1.35, 1.75, 2.15)), 
         col = c(viridis(3, alpha = 0.3)[2], viridis(3)[2],viridis(3, alpha = 0.3)[2],viridis(3)[2],
                 viridis(3)[2], viridis(3, alpha = 0.3)[2]),
...
#-------------------------------------------------------------------------------
# Coefficient plot - Model group 2 - Original
#-------------------------------------------------------------------------------
...
## Marginal effects plot (within interaction) - R & R
...
pdf("plots/Marginal_interflex_within_new.pdf", width = 12)
...
# 
# 
# ## Marginal effects plot (within interaction) - Original
...
# 
# m8_wb_exp <- lmer(log_ghg ~ demeaned_log_cc_outs + demeaned_vdem_poly + 
#                     demeaned_log_cc_outs * mean_vdem_poly +
...
# plot(out2_raw)
# 
# ## Marginal effects plot (between interaction) - R & R
# out2 <- interflex(Y = "log_ghg", 
#                   D = "mean_log_cc_outs", 
#                   X = "mean_vdem_poly", 
#                   treat.type = "continuous",
#                   Z = c("mean_log_pop", "mean_log_gdp_pc",
...
#                   ylim = c(-15, 15))
# 
# pdf("plots/Marginal_interflex_between_new.pdf", width = 12)
# 
# plot(out2,
#      pool = T,
#      xlab = "Democracy",
...
# dev.off()
# 
# ## Marginal effects plot (between interaction) - Original
...
min_dem_w <- min_dem - ((min_dem + max_dem)/2)
...
FD_FD_hat_w/1000 <- FD_dem_high_hat_w - FD_dem_low_hat_w
FD_FD_w <- FD_dem_high_w - FD_dem_low_w
CI_FD_FD_w/1000 <- quantile(FD_FD_w, c(0.025, 0.975))
...
#-------------------------------------------------------------------------------
pdf("plots/FD_interaction_within_new.pdf")

plot(y = 1,
     x = FD_dem_low_hat_w/1000,
     col = viridis(3)[1],
     ylim = c(0,3),
     xlim = c(-75,110),
    # xlab = expression("CO"[2]*" emissions [metric tons/capita]"),
     xlab = "GHG emissions [metric tons]",
     pch = 19,
     main = NULL, 
     bty = "n",
     ylab = "",
...
segments(y0 = 1, x0 = CI_FD_dem_low_w[1]/1000,
         y1 = 1, x1 = CI_FD_dem_low_w[2]/1000,
         col = viridis(3)[1],
         lwd = 12, cex = 1.5, lend = 1)

segments(y0 = 2, x0 = CI_FD_dem_high_w[1]/1000,
         y1 = 2, x1 = CI_FD_dem_high_w[2]/1000,
         col = viridis(3, alpha = 0.3)[1],
         lwd = 12, cex = 1.5, lend = 1)

points(y = 2,
       x = FD_dem_high_hat_w/1000,
       col = viridis(3)[1],
       xlab = "",
       pch = 19,
       main = NULL, 
...
text(x = FD_dem_high_hat_w/1000-0.35,
     y = 1.8,
...
## Plot First differences
#-------------------------------------------------------------------------------
pdf("plots/FD_interaction_between_new.pdf")

plot(y = 1,
     x = FD_hat_b_min_dem/1000,
     col = viridis(3)[2],
     ylim = c(0,3),
     xlim = c(-210, 170),
    # xlim = c(-16,12),
...
segments(y0 = 1, x0 = CI_FD_b_min_dem[1]/1000,
         y1 = 1, x1 = CI_FD_b_min_dem[2]/1000,
         col = viridis(3, alpha = 0.3)[2],
         lwd = 12, cex = 1.5, lend = 1)

segments(y0 = 2, x0 = CI_FD_b_max_dem[1]/1000,
         y1 = 2, x1 = CI_FD_b_max_dem[2]/1000,
         col = viridis(3)[2],
         lwd = 12, cex = 1.5, lend = 1)

points(y = 2,
       x = FD_hat_b_max_dem/1000,
       col = viridis(3)[2],
       xlab = "",
       pch = 19,
       main = NULL, 
...
text(x = FD_hat_b_max_dem/1000-0.35,
     y = 1.8,
     labels = c("More democratic"),
     cex = 1.1)
...
#-------------------------------------------------------------------------------
pdf("plots/FD_mean_outsource_between_new.pdf")

plot(y = 1.5,
     x = FD_hat_b_mean/1000,
     col = viridis(3)[2],
     ylim = c(0,3),
     xlim = c(-70,20),
     #xlim = c(-16,5.5),
...
segments(y0 = 1.5, x0 = CI_FD_b_mean[1]/1000,
         y1 = 1.5, x1 = CI_FD_b_mean[2]/1000,
         col = viridis(3, alpha = 0.3)[2],
         lwd = 12, cex = 1.5, lend = 1)
