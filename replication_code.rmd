...
# Controls
#-------------------------------------------------------------------------------
m2_c <- lm(log_cc_outs ~ log_pop + log_gdp_pc + log_gdp_pc_sq + # wdi_trade + wdi_gdpind + 
                      as.factor(year) + as.factor(ccode),
...
# Base model + controls
#-------------------------------------------------------------------------------
m3_outsource_c <- lm(log_cc_outs ~ vdem_polyarchy + log_pop + log_gdp_pc + log_gdp_pc_sq + # wdi_trade + wdi_gdpind +
                      as.factor(year) + as.factor(ccode),
                     data = data_s)
...
#-------------------------------------------------------------------------------
m4_restricted_c <- lm(log_cc_outs ~ vdem_polyarchy + log_pop + log_gdp_pc + log_gdp_pc_sq + # wdi_trade + wdi_gdpind
                       as.factor(year) + as.factor(ccode),
                      data = data_s[which(!is.na(data_s$log_pm25_pop)),])

stargazer(list(m1_outsource, m2_c, m3_outsource_c, m4_restricted_c), 
          type = "text", omit = c("ccode", "year"))
...
# Base model + controls
#-------------------------------------------------------------------------------
m7_c <- lm(log_pm25_pop ~ log_pop + log_gdp_pc + log_gdp_pc_sq + # wdi_trade + wdi_gdpind + 
            as.factor(year) + as.factor(ccode),
           data = data_s[which(!is.na(data_s$log_pm25_pop) &
...
stargazer(list(m5_out_dem, m6_out_dem_int, m7_c, m8_out_dem_int_c), 
          type = "text", omit = c("ccode", "year"))
...
## Create mean and demeaned variables: Model group 1
...
data_s <- data_s %>%
  ...
  mutate(
    ...
    interaction_demeaned = (log_cc_outs - mean(log_cc_outs, na.rm = T)) * (vdem_polyarchy - mean(vdem_polyarchy, na.rm = T)),
...
# Model group 2
#-------------------------------------------------------------------------------
m5_wb <- lmer(log_pm25_pop ~ demeaned_log_cc_outs + demeaned_vdem_poly + 
               mean_log_cc_outs + mean_vdem_poly +
               as.factor(year) + (1 | ccode),
              data = data_s)

m6_wb <- lmer(log_pm25_pop ~ demeaned_log_cc_outs + demeaned_vdem_poly +
...
m7_wb <- lmer(log_pm25_pop ~ demeaned_log_pop + demeaned_log_gdp_pc + demeaned_log_gdp_pc_sq + 
               mean_log_pop + mean_log_gdp_pc + mean_log_gdp_pc_sq + 
               as.factor(year) + (1 | ccode),
              data = data_s[which(!is.na(data_s$log_pm25_pop) &
                                  !is.na(data_s$log_cc_outs) &
...
m8_wb <- lmer(log_pm25_pop ~ demeaned_log_cc_outs + demeaned_vdem_poly + 
                interaction_demeaned +
...
m9_wb_high_dem <- lmer(log_pm25_pop ~ demeaned_log_cc_outs +  
                        mean_log_cc_outs + 
                        demeaned_log_pop + demeaned_log_gdp_pc + demeaned_log_gdp_pc_sq +
                        mean_log_pop +  mean_log_gdp_pc + mean_log_gdp_pc_sq +
                        as.factor(year) + (1 | ccode),
                       data = data_s[which(data_s$vdem_polyarchy >= median(data_s$vdem_polyarchy, na.rm = T)),])

m9_wb_low_dem <- lmer(log_pm25_pop ~ demeaned_log_cc_outs + 
                       mean_log_cc_outs + 
                        demeaned_log_pop + demeaned_log_gdp_pc + demeaned_log_gdp_pc_sq +
                        mean_log_pop +  mean_log_gdp_pc + mean_log_gdp_pc_sq +
                       as.factor(year) + (1 | ccode),
...
stargazer(list(m9_wb_high_dem , m9_wb_low_dem), type = "text", omit = c("year", "ccode"))
...
#-------------------------------------------------------------------------------
m1_random_is <- lmer(log_pm25_pop ~ log_cc_outs + 
                      as.factor(year) + 
                      (1  + log_cc_outs | cname),
                     data = data_s)
...
random_eff <- ranef(m1_random_is)$cname
re <- as.data.frame(random_eff[,2])
country_names <- rownames(random_eff)
re$region <- country_names
colnames(re)[1] <- "log_cc_outs_re"
...
re$region[which(re$region %like% "West Bank and Gaza")] <- "Palestina"
re$region[which(re$region %like% "Yemen")] <- "Yemen"
re$region[which(re$region %like% "Turkiye")] <- "Turkey"
re$region[which(re$region %like% "Syrian ")] <- "Syria"
re$region[which(re$region %like% "Slovak")] <- "Slovakia"
re$region[which(re$region %like% "Russian")] <- "Russia"
re$region[which(re$region %like% "Lao")] <- "Laos"
re$region[which(re$region %like% "Kyrgyz")] <- "Kyrgyzstan"
re$region[which(re$region %like% "Korea (the Republic of)")] <- "South Korea"
re$region[which(re$region %like% "Korea (the Democratic People's Republic of)")] <- "North Korea"
re$region[which(re$region %like% "Moldova")] <- "Moldova"
re$region[which(re$region %like% "Iran")] <- "Iran"
re$region[which(re$region %like% "Hong Kong SAR, China")] <- "Hong Kong"
re$region[which(re$region %like% "Netherlands")] <- "Netherlands"
re$region[which(re$region %like% "Philippines")] <- "Philippines"
re$region[which(re$region %like% "Gambia")] <- "Gambia"
re$region[which(re$region %like% "Egypt")] <- "Egypt"
re$region[which(re$region %like% "Czech")] <- "Czech Republic"
re$region[which(re$region %like% "Congo, Dem. Rep.")] <- "Democratic Republic of the Congo"
re$region[which(re$region %like% "Congo, Rep.")] <- "Republic of Congo"
re$region[which(re$region %like% "Macao SAR, China")] <- "Macao"
re$region[which(re$region %like% "Brunei Darussalam")] <- "Brunei"
re$region[which(re$region %like% "Taiwan Province of China")] <- "Taiwan"
re$region[which(re$region %like% "St. Kitts and Nevis")] <- "Saint Kitts and Nevis"
re$region[which(re$region %like% "St. Lucia")] <- "Saint Lucia"
re$region[which(re$region %like% "St. Vincent and the Grenadines")] <- "Saint Vincent and the Grenadines"
re$region[which(re$region %like% "North Macedonia")] <- "Macedonia"
re$region[which(re$region %like% "Eswatini")] <- "Swaziland"
re$region[which(re$region %like% "Micronesia")] <- "Micronesia"
re$region[which(re$region %like% "Cabo Verde")] <- "Cape Verde"
re$region[which(re$region %like% "Bahamas")] <- "Bahamas"
re$region[which(re$region %like% "Virgin Islands")] <- "Virgin Islands U.S."
re$region[which(re$region %like% "Venezuela")] <- "Venezuela"
re$region[which(re$region %like% "Yemen")] <- "Yemen"
re$region[which(re$region %like% "Viet Nam")] <- "Vietnam"
re$region[which(re$region %like% "Bolivia")] <- "Bolivia"
re$region[which(re$region %like% "Niger")] <- "Niger"
re$region[which(re$region %like% "United Kingdom")] <- "United Kingdom"
re$region[which(re$region %like% "United States")] <- "United States"
re$region[which(re$region %like% "Moldova")] <- "Moldova"
re$region[which(re$region %like% "Tanzania")] <- "Tanzania"
re$region[which(re$region %like% "Central African")] <- "Central African Republic"

## join with random effects
world <- left_join(world, re, by = "region")

#-------------------------------------------------------------------------------
# Plot on map
#-------------------------------------------------------------------------------
pdf("Plots/Map_coef_rirs.pdf", width = 12)

world_re <- world_re[which(!is.na(world$l))]
ggplot() +
geom_map(data = world, map = world,
         aes(long, lat, map_id = region, fill = log_cc_outs_re)) +
