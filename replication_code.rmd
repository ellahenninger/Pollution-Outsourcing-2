...
m8_out_dem_int_c <- lm(log_pm25_pop ~ log_cc_outs + vdem_polyarchy + 
                        log_cc_outs*vdem_polyarchy +
                        log_pop + log_gdp_pc + log_gdp_pc_sq + # wdi_trade + wdi_gdpind + 
                        as.factor(year) + as.factor(ccode),
                       data = data_s)
...
meff <- margins(m8_out_dem_int_c, variables = "log_cc_outs", at = list(vdem_polyarchy = seq(0.016, 0.926, length.out = 30)))
